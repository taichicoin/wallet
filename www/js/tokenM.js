...
function renderTokenList(mainSymbol, mainDisplay, tokens) {
    ...
    if (!container) return;
    const isBSC = (currentChainId === 56 || currentChainId === 97);
    const mainIcon = isBSC ? '<img src="imagine/bnb.jpg" class="token-icon" style="object-fit:cover;">' : '<div class="token-icon">🔷</div>';
    let html = `<div class="token-item"><div class="token-info">${mainIcon}<div><div class="token-name">${mainSymbol}</div><div class="token-symbol">主网币</div></div></div><div class="token-balance"><div class="amount">${mainDisplay}</div><div class="fiat">$0.00</div></div></div>`;
    tokens.forEach(t => {
        html += `<div class="token-item"><div class="token-info"><div class="token-icon">${(t.symbol||'T')[0].toUpperCase()}</div><div><div class="token-name">${t.symbol}</div><div class="token-symbol">${t.address.substring(0,8)}...</div></div></div><div class="token-balance"><div class="amount">${t.displayBalance}</div><div class="fiat">$0.00</div></div></div>`;
    });
...
async function getCustomTokenBalances() {
    ...
    for (const t of customTokens[currentChainId]) {
        try {
            ...
            results.push({ address: t.address, symbol: sym || 'TOKEN', decimals: dec || 18, displayBalance: formatBalance(bal, dec) });
        } catch (e) {
            results.push({ address: t.address, symbol: '?', decimals: 18, displayBalance: '查询失败' });
        }
    }
    return results;
}
...
function confirmImportToken() {
    ...
    customTokens[currentChainId].push({ address: addr });
