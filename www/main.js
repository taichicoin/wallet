...
// ========== 余额格式化（统一规则：小数点后连续0≥7个显示0，否则保留最多6位有效小数） ==========
...
async function loadBalances() {
    ...
    // 主网币余额
    let mainBalanceRaw, mainDisplay;
    try {
        mainBalanceRaw = await provider.getBalance(wallet.address);
    ...
    // 代币列表（若未配置 API Key 或网络问题，则仅显示主链币）
    let tokens = [];
    // 简单起见，不强制要求 API，代币功能可后续添加
    // 此处仅保留主链币显示
    renderTokenList(chain.symbol, mainDisplay, tokens);
}
