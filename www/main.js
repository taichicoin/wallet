...
function renderTokenList(mainSymbol, mainBalanceDisplay, tokens) {
    ...
    // 主网币图标：BSC 主网/测试网 使用本地图片
    let mainIconHtml;
    if (currentChainId === 56 || currentChainId === 97) {
        mainIconHtml = `<img src="imagine/bnb.jpg" class="token-icon" style="object-fit:cover;">`;
    } else {
...
function showImportTokenInput() {
    ...
    div.id = 'customTokenInput';
    div.style.marginTop = '12px';
    div.innerHTML = `
        <input type="text" id="newTokenAddress" placeholder="代币合约地址 (0x...)" style="background:var(--card); border:1px solid var(--border); color:var(--text); padding:10px; border-radius:8px; width:100%;" />
        <div style="display:flex; gap:8px; margin-top:8px;">
            <button class="btn-primary" onclick="confirmImportToken()" style="flex:1;">添加</button>
            <button class="btn-outline" onclick="cancelImportToken()" style="flex:1;">取消</button>
        </div>
    `;
...
function confirmImportToken() {
    ...
    if (!customTokens[currentChainId]) customTokens[currentChainId] = [];
    if (customTokens[currentChainId].some(t => t.address.toLowerCase() === address.toLowerCase())) {
        showToast('该代币已添加');
        return;
    }
    customTokens[currentChainId].push({ address });
...
function cancelImportToken() {
    const el = document.getElementById('customTokenInput');
    if (el) el.remove();
}
