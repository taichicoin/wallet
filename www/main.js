...
function closeUnlockModal() { document.getElementById('unlockModal').classList.remove('active'); pendingUnlockIndex = null; }
function updatePayUnlockButton() {
    ...
    if (passwordInput.value.trim().length > 0) {
        unlockBtn.textContent = '解锁'; unlockBtn.onclick = executeUnlock; unlockBtn.className = 'btn-primary';
    } else {
        const bioEnabled = localStorage.getItem('biometric_enabled') === 'true';
        if (bioEnabled) {
            unlockBtn.textContent = '🔐 指纹解锁'; unlockBtn.onclick = startPayBiometric; unlockBtn.className = 'btn-outline';
        } else {
            unlockBtn.textContent = '解锁'; unlockBtn.onclick = executeUnlock; unlockBtn.className = 'btn-primary';
        }
    }
...
function startPayBiometric() {
    if (!window.android || typeof window.android.biometricAuth !== 'function') { showToast('当前设备不支持指纹解锁'); return; }
        ...
        if (window.__biometricContext === 'pay') {
            if (success) {
                if (window.android && typeof window.android.getSecret === 'function') {
                    ...
                    showToast('安全存储不可用'); window.__biometricContext = null; window.__onBiometricResult = originalCallback;
                }
            } else {
                ...
                window.__biometricContext = null; window.__onBiometricResult = originalCallback;
            }
        }
    };
...
window.__onSecretRetrieved = function(key, value) {
    ...
    } else { showToast('未找到保存的密码，请先手动输入一次'); }
};
async function executeUnlock() {
    ...
    try {
        ...
        updateUI(); loadBalances(); closeUnlockModal(); showToast('解锁成功');
...
async function importWallet() {
    ...
    try {
        ...
        showToast('导入成功，请解锁'); openUnlockModal(newIndex);
...
function deleteAccount() {
    ...
    accounts.splice(parseInt(idx), 1); saveAccounts(); wallet = null; updateUI();
...
async function switchChain(chainId) {
    ...
    try {
        ...
        updateNetworkUI(); loadBalances(); closeNetworkModal();
...
function updateNetworkUI() {
    ...
    if (logoImg) {
        if (chain.logo) { logoImg.src = chain.logo; logoImg.style.display = 'block'; }
...
function renderChainList() {
    ...
    container.innerHTML = chains.map(c => {
        const iconHtml = c.logo ? `<img src="${c.logo}" class="chain-icon" style="object-fit:cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />` : '';
        const fallbackHtml = c.logo ? `<div class="chain-icon" style="display:none; background:#333; align-items:center; justify-content:center;">${c.symbol[0]}</div>` : `<div class="chain-icon" style="background:#333; display:flex; align-items:center; justify-content:center;">${c.symbol[0]}</div>`;
        return `<div class="chain-list-item ${c.id === currentChainId ? 'active' : ''}" onclick="switchChain(${c.id})">${iconHtml}${fallbackHtml}<div><div class="chain-name">${c.name}</div><div class="chain-symbol">${c.symbol}</div></div>${c.id === currentChainId ? '<span style="margin-left:auto;color:var(--green);">✔</span>' : ''}</div>`;
    }).join('');
}

// ========== 发送 ==========
function openSendModal() { if (!wallet) return showToast('请先解锁'); document.getElementById('sendModal').classList.add('active'); updateTokenSelect(); }
...
function startApp() {
    ...
    if (!document.getElementById('exportKeyModal')) injectExportKeyUI();
}
...
function injectExportKeyUI() {
    ...
    if (settingsContent && !document.getElementById('exportKeyBtn')) {
        const btn = document.createElement('button'); btn.id = 'exportKeyBtn'; btn.className = 'btn-outline';
        btn.textContent = '导出私钥'; btn.style.marginTop = '8px'; btn.onclick = openExportPrivateKey;
        const deleteBtn = settingsContent.querySelector('.btn-outline');
        if (deleteBtn) deleteBtn.insertAdjacentElement('afterend', btn); else settingsContent.appendChild(btn);
    }
    if (!document.getElementById('exportKeyModal')) {
        const modalHTML = `<div class="modal-overlay" id="exportKeyModal"><div class="modal-content"><div class="modal-title"><span>导出私钥</span><span onclick="closeExportKeyModal()" class="close-btn">&times;</span></div><input type="password" id="exportKeyPassword" placeholder="输入支付密码" /><button class="btn-primary" onclick="confirmExportKey()">确认</button><div id="exportKeyStatus" class="tx-status" style="display:none;"></div></div></div>`;
        document.body.insertAdjacentHTML('beforeend', modalHTML);
        document.getElementById('exportKeyModal').addEventListener('click', function(e) { if (e.target === this) closeExportKeyModal(); });
    }
}
function openExportPrivateKey() {
    if (!wallet) { showToast('请先解锁钱包'); return; }
    document.getElementById('exportKeyPassword').value = ''; document.getElementById('exportKeyStatus').style.display = 'none';
...
async function confirmExportKey() {
    ...
    try {
        ...
        statusEl.style.display = 'block'; statusEl.className = 'tx-status success'; statusEl.innerText = '私钥：' + privateKey;
        try { await navigator.clipboard.writeText(privateKey); statusEl.innerText += '\n(已复制到剪贴板)'; } catch (e) {}
    } catch (e) { statusEl.style.display = 'block'; statusEl.className = 'tx-status error'; statusEl.innerText = '密码错误或数据损坏'; }
}
