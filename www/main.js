...
function updateTokenSelect() {
    ...
    let options = '';
    // 主网币选项
    options += `<option value="native" data-decimals="18">${chain.symbol} (主网币)</option>`;
    // 已导入的代币
    if (typeof getImportedTokens === 'function') {
        const tokens = getImportedTokens();
        tokens.forEach(t => {
            options += `<option value="${t.address}" data-decimals="${t.decimals || 18}">${t.symbol}</option>`;
        });
    }
...
async function executeSend() {
    ...
    const selectedValue = tokenSelect.value;        // "native" 或合约地址
