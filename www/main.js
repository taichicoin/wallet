...
const chains = [
    ...
    { id: 97, name: "BSC Testnet", symbol: "tBNB", rpcs: ["https://data-seed-prebsc-1-s1.binance.org:8545","https://bsc-testnet.publicnode.com"] },
...
function renderChainList() {
    ...
    container.innerHTML = chains.map(c => `
        <div class="chain-list-item ${c.id === currentChainId ? 'active' : ''}" onclick="switchChain(${c.id})">
            <div class="chain-icon">⛓️</div><div><div class="chain-name">${c.name}</div><div class="chain-symbol">${c.symbol}</div></div>
    ...
    handle: function(method, paramsJson, callbackId) {
        ...
        const respond = (error, result) => {
            ...
            if (window.android && window.android.returnResult) {
                ...
                console.warn('缺少原生 returnResult 接口，DApp 请求将无响应');
            }
        };
...
// 原生层会用这个函数来接收钱包处理结果（备选方案，一般不用）
window._walletResponseCallback = function(id, error, result) {
    if (window.android && window.android.returnResult) {
        window.android.returnResult(id, JSON.stringify(error), JSON.stringify(result));
    }
};
