...
// 原生指纹结果回调（用于支付解锁）
window.__onPayBiometricResult = function(success, errorMsg) {
    if (success) {
        // 从安全存储获取密码（原生会调用 getSecret 返回密码）
        if (window.android && typeof window.android.getSecret === 'function') {
            window.android.getSecret('wallet_unlock_pwd');
        } else {
            // 降级：尝试从 localStorage 读取（不安全，但可工作）
            const savedPwd = localStorage.getItem('temp_wallet_pwd');
            if (savedPwd) {
                document.getElementById('unlockPassword').value = savedPwd;
                executeUnlock();
            } else {
                showToast('未找到保存的密码，请先输入一次');
            }
        }
    } else {
...
async function executeUnlock() {
    ...
    try {
        ...
        // 解锁成功后，如果开启了指纹，安全存储密码
        if (localStorage.getItem('biometric_enabled') === 'true') {
            if (window.android && typeof window.android.saveSecret === 'function') {
                window.android.saveSecret('wallet_unlock_pwd', password);
            } else {
                // 降级 localStorage（临时）
                localStorage.setItem('temp_wallet_pwd', password);
            }
        }
    } catch (e) {
        statusEl.style.display = 'block';
