...
function notifyDappAccountsChanged() {
    if (window.android && typeof window.android.updateAccounts === 'function') {
        window.android.updateAccounts(wallet ? wallet.address : '');
    }
}
...
function notifyDappChainChanged() {
    if (window.android && typeof window.android.updateChainId === 'function') {
        window.android.updateChainId(currentChainId);
    }
}
...
// ========== DApp 钱包桥接（正确区分 eth_accounts 和 eth_requestAccounts） ==========
window.__walletAPI = {
        ...
        (async () => {
            try {
                if (method === 'eth_accounts') {
                    // 直接返回当前解锁地址，不弹出解锁
                    respond(null, wallet ? [wallet.address] : []);
